...
@section('content')
  <div class="min-vh-100 d-flex flex-column position-relative">
    ...
    <div id="reader" class="container-fluid mt-5 pt-4" data-comic-id="{{ $comic->id }}">
      @php
        $pageUrls = $chapter->pages->map(function ($p) {
          return $p->image_url;
        })->toArray();
      ...
      @if($total)
        <style>
          ...
          .reader-side {
            ...
            box-shadow: -6px 0 18px rgba(0, 0, 0, .05);
            overflow: auto;
          ...
          .reader-layout.open .reader-side {
            transform: translateX(0);
          }

          .reader-layout.open .reader-main {
            margin-right: 360px;
          }

          .reader-side .star-inputs .fa-star {
            cursor: pointer;
            color: #ddd;
          }

          .reader-side .star-inputs .fa-star.selected {
            color: #ffc107;
          }
        ...
        <div class="reader-layout" id="readerLayout">
          <div class="reader-main">
            <div class="position-fixed top-50 start-0 end-0 translate-middle-y d-flex justify-content-between px-3"
              style="z-index: 1000;" aria-hidden="true">
          ...
          <aside class="reader-side" id="readerSide" aria-hidden="true">
            ...
            <div>
              ...
              <div id="commentsList" class="mb-3">
                @foreach($chapter->comments->sortByDesc('created_at') as $comment)
                  <div class="card mb-3 pb-2 border-bottom">
                    ...
                    <div class="text-break">{{ $comment->body }}</div>
                  </div>
                @endforeach
              </div>

              @auth
                <form id="commentForm" action="{{ route('comments.store', [$comic, $chapter]) }}" method="POST">
                  @csrf
                  <input type="hidden" name="chapter_id" value="{{ $chapter->id }}">
                  <div class="mb-2">
          ...
          <div class="container">
            <div class="d-flex align-items-center justify-content-between w-100">
              <div class="d-flex align-items-center gap-3">
                ...
                <div class="small text-muted">Page <span id="currentPage">1</span> of <span
                    id="totalPages">{{ $total }}</span></div>
              </div>
